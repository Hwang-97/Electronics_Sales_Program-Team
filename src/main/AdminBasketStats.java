...
public class AdminBasketStats {
	...
	private static void getCategory2() {
		...
		for(int i=0; i<AdminStats.basketProductStats.size();i++) {
			String key =AdminStats.basketProductStats.get(i).getProductCategory2Name();
	...
	private static void getCategory1() {
		...
		for(int i=0; i<AdminStats.basketProductStats.size();i++) {
			String key =AdminStats.basketProductStats.get(i).getProductCategory1Name();
			if(map.containsKey(key)) {
				map.put(key, map.get(key)+1);
			}else {
				map.put(key, 1);
	...
	private static void getAge() {
		...
		tap();
		for(int i=0;i<AdminStats.basketMemberstat.size();i++) {
			int year = 0;
			String total = AdminStats.basketMemberstat.get(i).getRegistrationNumber().substring(0,2);
	...
	public static void getGender() {
		...
		int count=0;
		for(int i=0;i<AdminStats.basketMemberstat.size();i++) {
			if(AdminStats.basketMemberstat.get(i).getGender().equals("M")) {
				count++;
			}
		}
		...
		count=0;
		for(int i=0;i<AdminStats.basketMemberstat.size();i++) {
			if(AdminStats.basketMemberstat.get(i).getGender().equals("F")) {
				count++;
			}
		}
