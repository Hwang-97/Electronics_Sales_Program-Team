...
public class SearchNonMember {
	public static boolean main() {
		...
		while (loop) {
			if (Main.nowLogin) {
				...
				System.out.print("선택: ");
				int sel = Main.inputUserNum(0, Main.nonMemberList.get(Main.nonMemberList.size()-1).getMemberNumber());
				Main.scan.nextLine();
	...
	private static void menuSearchMember() {
		...
		System.out.println("───────────────────────────────────────────── [ 개인 비회원 조회 ] ────────────────────────────────────────────");
		System.out.println("0. 뒤로가기");
		System.out.println("최대 " + Main.nonMemberList.get(Main.nonMemberList.size()-1).getMemberNumber()+"까지");
