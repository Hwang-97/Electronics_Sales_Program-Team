...
public class MemberList {
	private static String path = "txtData\\회원데이터.txt";
	...
	public static List<Member> load() throws Exception {
		...
		for (Member m : memberList) {
			...
			m.setOrderList(mOrderList);
		}

		// 장바구니 데이터 저장
		List<Basket> basketList = new ArrayList<Basket>();
		basketList = BasketList.load();
		for (Member m : memberList) {
			List<Basket> mBasketList = new ArrayList<Basket>();
			for (Basket b : basketList) {
				if (m.getMemberNumber() == b.getMemberNum()) {
					mBasketList.add(b);
				}
			}
			m.setBasketList(mBasketList);
		}
